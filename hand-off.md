...
**最後更新：** 2026-09-01 · 狀態對應 commit `cd0e307`（最後一次改變專案狀態的 commit）

---

## 一句話狀態

核心引擎與網頁工具都已完成並通過測試，**CI 全綠**；卡在 GitHub Pages 尚未啟用，demo 網址還沒生出來，因此還不能送 Codex for OSS 申請。
...
貼 `good first issue` / `help wanted` 標籤：路徑變形（path morph）、hover 動畫、scroll 動畫、Lottie 漸層支援、dotLottie 輸出。
